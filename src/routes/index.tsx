...
function Logo() {
  return <div className="group flex items-center gap-3 text-[22px] font-[750] tracking-[-1px] max-sm:text-[19px]" aria-label="Heyday.Money">
    <svg className="size-[43px] overflow-visible transition-transform duration-300 group-hover:-rotate-7 max-sm:size-[37px]" viewBox="0 0 1024 1024" aria-hidden="true">
...
function FeatureNote({ position, icon: Icon, tone, kicker, title }: {
  ...
  return <div className={`absolute z-2 flex animate-appear items-center gap-3 rounded-xl border border-card-border bg-card p-3 shadow-[0_7px_30px_#30392a08] backdrop-blur-[14px] transition duration-300 hover:-translate-y-1 max-lg:gap-2 max-lg:p-2.5 max-sm:gap-1.5 max-sm:rounded-lg max-sm:p-2 ${position}`}>
    ...
    <div><span className="block text-[7px] font-semibold tracking-[1.35px] text-muted max-lg:text-[6px] max-sm:text-[5px] max-sm:tracking-[.7px]">{kicker}</span><strong className="mt-1 block text-xs font-[650] max-lg:text-[10px] max-sm:mt-0.5 max-sm:text-[8px]">{title}</strong></div>
  </div>
}
...
function Landing() {
  ...
  return <div className="page-shell relative grid h-svh min-h-[600px] grid-rows-[80px_minmax(0,1fr)_80px] overflow-hidden bg-page text-ink transition-colors duration-700 max-sm:min-h-[640px] max-sm:grid-rows-[68px_minmax(0,1fr)_68px]">
    <a className="absolute -top-16 left-5 z-20 rounded bg-ink p-2.5 text-page focus:top-2.5" href="#main">Skip to content</a>
    ...
    <header className="relative z-2 flex h-full items-center justify-between px-[4.4%] max-sm:px-[22px]">
      <Logo />
      <div className="flex items-center gap-3"><span className="text-[11px] tracking-[.2px] text-muted max-lg:hidden">A brighter perspective</span><button className="relative flex h-[39px] w-[78px] items-center justify-between gap-4 rounded-full border border-line bg-[#eaece5] px-2.5 dark:bg-[#16191d]" disabled={!ready} onClick={toggleTheme} aria-label="Night mode" role="switch" aria-checked={theme === 'dark'}><span className="absolute top-1 left-1 size-[29px] rounded-full bg-white shadow-sm transition-all duration-400 dark:translate-x-[38px] dark:bg-brand" /><Sun size={17} className="z-1 shrink-0 text-[#756534]" /><Moon size={17} className="z-1 shrink-0 text-[#888e88] dark:text-white" /></button></div>
    </header>
    ...
    <main id="main" className="main-content relative mx-auto grid h-full max-h-[850px] min-h-0 w-full self-center grid-rows-[auto_minmax(0,1fr)_auto]">
      <section className="relative z-1 flex animate-appear flex-col items-center px-6 pt-2 text-center max-sm:px-5" aria-labelledby="headline">
        <div className="mb-4 inline-flex items-center gap-2 text-[10px] font-semibold tracking-[2px] text-muted max-sm:mb-3 max-sm:text-[8px] max-sm:tracking-[1.4px] short:mb-2.5"><span className="size-1.5 rounded-full bg-brand" /> A LITTLE CLARITY. A LOT MORE FREEDOM.</div>
        <h1 id="headline" className="relative inline-block text-[clamp(40px,4.5vw,67px)] leading-[1.1] font-[650] tracking-[-3.6px] max-lg:text-[49px] max-lg:tracking-[-2.8px] max-sm:text-[clamp(39px,10.6vw,56px)] max-sm:leading-[1.13] max-sm:tracking-[-2.3px] short:text-5xl max-sm:short:text-4xl">Your money,<br className="hidden max-sm:block" /> <span className="relative inline-block text-brand dark:text-[#b58bff]">Your Heyday!
        ...
        <p className="mx-auto mt-4 text-sm leading-[1.8] text-muted max-sm:max-w-[315px] max-sm:text-xs short:mt-2.5 short:text-[13px] max-sm:short:text-xs max-sm:short:leading-[1.6]">A clear picture of your cash flow, spending, and subscriptions.<br className="max-sm:hidden" /> Take back control of your finances. Make room for living.</p>
      </section>

      <section className="island-scene relative mx-auto h-full min-h-0 w-full max-w-[1000px] [container-type:size]" aria-label="Your own little world of financial freedom">
        ...
        <div className="celestial-orbit pointer-events-none absolute inset-y-0 inset-x-[3%]" key={skyCycle} data-transition={skyCycle > 0} aria-hidden="true">
          <div className="celestial-body day-sun absolute top-0 left-0 size-16 text-[#d7ad20] drop-shadow-[0_0_12px_#fed72865] max-sm:size-11"><Sun className="size-full" strokeWidth={1.5} /></div>
        ...
        <div className="island-float absolute top-1/2 left-1/2 w-[min(66%,calc(150cqh-24px))] max-w-[700px] -translate-1/2 animate-float max-sm:w-[min(100%,calc(150cqh-24px))]"><img className="island-image relative z-1 block h-auto w-full transition-[filter] duration-1000 dark:brightness-[.78] dark:saturate-[.8] dark:drop-shadow-[0_0_35px_#8942fe20]" src="/images/island.png" alt="A peaceful floating island with a purple-roofed home, green trees, and a little pond" width="1536" height="1024" fetchPriority="high" /></div>
        <FeatureNote position="left-[4%] top-[30%] -rotate-4 max-sm:top-[23%]" icon={ArrowUpRight} tone="bg-brand/6 text-brand" kicker="SEE THE BIG PICTURE" title="Cash flow, made clear" />
        <FeatureNote position="right-[3%] top-[35%] rotate-4 max-sm:top-[30%]" icon={ArrowDownLeft} tone="bg-accent/15 text-[#a48712]" kicker="KNOW WHERE IT GOES" title="Mindful spending" />
        <FeatureNote position="right-[3%] top-[70%] -rotate-3 max-sm:top-[75%]" icon={RefreshCw} tone="bg-[#7bad85]/10 text-[#679269]" kicker="STAY ONE STEP AHEAD" title="Subscriptions in sight" />
      ...
      <section className="download-area relative z-2 animate-appear px-5 pt-2 pb-2 text-center [animation-delay:200ms]" aria-label="Download Heyday Money">
        <p className="mb-4 text-[13px] font-[550] max-sm:mb-3 max-sm:text-xs short:mb-2.5">A fresh start for your finances.</p>
        <div className="flex flex-wrap justify-center gap-3 max-sm:gap-2">
          {(['macOS', 'Windows'] as const).map(platform => <button className={`flex min-w-48 items-center gap-3 rounded-xl border px-4 py-3 text-left transition duration-250 hover:-translate-y-0.5 hover:shadow-lg active:translate-y-0 max-sm:min-w-[155px] max-sm:gap-2 max-sm:px-3 short:py-2 [&>svg:first-child]:size-[27px] ${platform === 'macOS' ? 'border-transparent bg-brand text-white shadow-brand/10 shadow-md' : 'border-line bg-card text-ink'}`} key={platform} onClick={() => setNotice(`Heyday for ${platform} is coming soon. Your fresh start is on its way.`)} aria-describedby="download-note"><PlatformIcon platform={platform} /><span><small className="block text-[9px] leading-[1.4] opacity-75">Download for</small><strong className="block text-base leading-[1.4] font-semibold max-sm:text-sm">{platform}</strong></span><ArrowUpRight className="ml-auto opacity-65 max-sm:size-3.5" size={18} /></button>)}
        </div>
        <p id="download-note" className="mt-3 flex items-center justify-center gap-1.5 text-[10px] text-muted short:mt-2"><span className="size-[5px] rounded-full bg-[#a5ad97]" /> macOS & Windows · Coming soon</p>
      </section>
    </main>

    <footer className="site-footer relative mx-[4.4%] flex h-full items-center justify-between gap-5 border-t border-line text-[10px] text-muted max-sm:flex-col max-sm:justify-center max-sm:gap-2">
      <span className="text-[10px] max-sm:text-[9px]">Don’t let money control you. <strong className="font-[550] text-ink">Play your game.</strong></span>
      <span className="absolute left-1/2 flex -translate-x-1/2 items-baseline gap-2 whitespace-nowrap text-[10px] max-lg:hidden"><strong className="font-serif text-[15px] text-ink">heyday</strong> <span>/ˈheɪdeɪ/</span> <em>Your Prime, Defined.</em></span>
      <span className="text-[9px] max-sm:text-[8px]">© {new Date().getFullYear()} Heyday.Money</span>
    </footer>
    <div className={`fixed bottom-6 left-1/2 z-10 flex w-max max-w-[calc(100%-32px)] -translate-x-1/2 items-center gap-3 rounded-xl bg-ink p-4 text-[13px] text-page shadow-xl transition duration-200 ${notice ? 'pointer-events-auto translate-y-0 opacity-100' : 'pointer-events-none translate-y-6 opacity-0'}`} role="status" aria-live="polite">{notice && <><Check size={19} className="shrink-0 text-[#b6d8a1]" /><span>{notice}</span><button className="grid place-items-center p-1" onClick={() => setNotice('')} aria-label="Dismiss notification"><X size={18} /></button></>}</div>
  </div>
}
